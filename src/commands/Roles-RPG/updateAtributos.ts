    ...
    run: async ({interaction}) => {
        ...
        if (ficha) {
            ...
            try {
                ...
                return await interaction.reply({ content: `Atributos Atualizados para ${nome}!`});

            } catch (err) {
                console.log(err);
                return await interaction.reply({ content: "Algo de errado ocorreu ao tentar inserir os Atributos"})
            }
        }else{
            return await interaction.reply({ content: "Esta Ficha não foi encontrada"});
        }
    }
